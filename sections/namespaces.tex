...
\begin{frame}{Namespaces}{\texttt{using} declarations and directives}
	...
	\bigskip
	A \texttt{using} \textbg{declaration} introduces a \textbg{single name} into the current scope:
	\begin{itemize}
		\item \texttt{using std::vector;} — after this, \texttt{vector} alone refers to \texttt{std::vector}.
	\end{itemize}
	\bigskip
	A \texttt{using namespace} \textbg{directive} imports \textbg{all names} from a namespace:
	\begin{itemize}
		\item \texttt{using namespace std;} (convenient for this example but \textbg{dangerous, never do this}).
	\end{itemize}
\end{frame}
