...
% ### RAII ###
\section{RAII}
\graphicspath{{figs/raii/}}

% --- The resource problem ---
\begin{frame}{RAII}{The resource problem}
	\justifying
	A \textbg{resource} is anything that must be \textbf{acquired} before use and \textbf{released} after use:
	\begin{itemize}
		\item \textbg{memory} allocated on the heap (\texttt{malloc}/\texttt{free}, \texttt{new}/\texttt{delete});
	...
	\begin{alertblock}{The classic bug}
		\justifying
		An early \texttt{return}, a \texttt{goto}, or an unhandled error condition leaves a resource unreleased.
		In a long-running system this causes \textbg{resource leaks} that accumulate over time.
	\end{alertblock}
\end{frame}
...
\begin{frame}{RAII}{Resource Acquisition Is Initialization}
	...
	\justifying
	\bigskip
	Because the destructor is \textbg{guaranteed to run} when an object goes out of scope, resource release becomes \textbf{automatic and unconditional}:
	\begin{itemize}
...
\begin{frame}{RAII}{RAII in practice}
	...
	\bigskip
	You have already seen it in your OS course, just without the name:
	\begin{itemize}
	...
	\end{itemize}
	\bigskip
	We will build one from scratch in the code examples to make the mechanism concrete.
\end{frame}
