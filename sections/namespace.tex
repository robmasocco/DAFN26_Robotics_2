...
\begin{frame}{Namespaces}{The naming collision problem}
	...
	\bigskip
	In small programs this is manageable. In large codebases that integrate \textbg{multiple libraries} — written by different authors, at different times — it becomes a problem:
	\begin{itemize}
		\item two libraries may define a function named \texttt{init()}, a struct named \texttt{Node}, or a constant named \texttt{MAX\_SIZE};
		\item the linker cannot resolve the collision;
		\item C works around this with \textbg{name prefixes} by convention: \texttt{pthread\_mutex\_t}, \texttt{SDL\_Init()}, \texttt{PTHREAD\_MUTEX\_INITIALIZER}...
	\end{itemize}
	\bigskip
	\begin{block}{}
		\centering
		C++ solves this with \textbf{namespaces} — a language-level partitioning of the global name space.
	\end{block}
\end{frame}
...
\begin{frame}{Namespaces}{\texttt{using} declarations and directives}
	\justifying
	Writing the full namespace prefix every time can become verbose.\\
	C++ provides two mechanisms to shorten names.\\
	\bigskip
	A \textbg{\texttt{using} declaration} introduces a \textbf{single name} into the current scope:
	\begin{itemize}
		\item \texttt{using std::vector;} — after this, \texttt{vector} alone refers to \texttt{std::vector}.
	\end{itemize}
	\bigskip
	A \textbg{\texttt{using namespace} directive} imports \textbf{all names} from a namespace:
	\begin{itemize}
		\item \texttt{using namespace std;} — convenient in small examples, \textbg{dangerous in production code}.
	\end{itemize}
	\bigskip
	\begin{alertblock}{Never \texttt{using namespace} in a header file}
		\justifying
		A \texttt{using namespace} directive in a \texttt{.hpp} file \textbg{pollutes the namespace} of every translation unit that includes it, potentially causing the exact collisions namespaces were designed to prevent.
		Confine it to \texttt{.cpp} files, and only use it deliberately.
	\end{alertblock}
\end{frame}
...
\begin{frame}{Namespaces}{Taming long names}
	\justifying
	Namespaces and templates combined can produce \textbg{very long type names}.\\
	C++ provides two ways to create \textbf{aliases}:
	\begin{itemize}
		\item \texttt{typedef} inherited from C: \texttt{typedef std::vector<int> IntVec;};
		\item \textbg{\texttt{using}} alias (preferred in modern C++): \texttt{using IntVec = std::vector<int>;}.
	\end{itemize}
	\bigskip
